...
@php
    // Normalize image links to respect current APP_URL and strip any hardcoded hosts or Blade-like placeholders
    $appUrl = rtrim(config('app.url'), '/');
    $normalizedImages = collect($options['images'] ?? [])->map(function ($image) use ($appUrl) {
        $link = $image['link'] ?? null;

        if ($link) {
            $raw = trim($link);

            // Decode percent-encoded content first
            ...
            if (preg_match('/^\s*\{\{\s*(.*?)\s*\}\}\s*$/', $raw, $mm)) {
                $raw = $mm[1] ?? $raw;
            }

            // If content contains a Blade-like helper e.g. "{{ url('/clothes') }}", extract the inner path
            if (preg_match("/url\\((['\"])\s*(.*?)\s*\\1\\)/", $raw, $m)) {
                $raw = $m[2] ?? $raw;
            }

            // If a full URL or protocol-relative, normalize localhost/127 to APP_URL
            ...
            if ($startsWith($raw, ['http://', 'https://', '//'])) {
                ...
                if ($host === 'localhost' || $host === '127.0.0.1') {
                    ...
                    if (preg_match('#^/[A-Za-z0-9\-]+$#', $path)) {
                        $raw = route('shop.search.index', ['featured' => 1, 'sort' => 'name-desc']);
                    } else {
                        $raw = rtrim($appUrl, '/') . $path;
                    }
                }
            } else {
                // Handle scheme-less localhost or relative paths
                if (preg_match('#^(localhost|127\.0\.0\.1)(?::\d+)?(/.*)$#i', $raw, $mm)) {
                    $path = $mm[2] ?: '/';
                } else {
                    $parsed = @parse_url($raw);
...
<v-carousel :images="{{ json_encode($normalizedImages) }}">
